...
export default function CafedraFormModal({
    cafedra = null,
    institute,
    onClose,
    onSubmit,
    ...
    const [name, setName] = useState(cafedra?.name || "");
    const [error, setError] = useState("");

    const handleSubmit = (e) => {
        e.preventDefault();
        if (!name.trim()) {
            setError("Название кафедры обязательно");
            return;
        }
        onSubmit(name);
    };
            ...
            <div className="bg-white rounded-xl p-8 w-full max-w-md shadow-xl">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">
                        {cafedra ? "Редактировать кафедру" : "Добавить кафедру"}
                    </h2>
                    <button
                        onClick={onClose}
